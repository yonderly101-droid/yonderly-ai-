...
function repositoryFromPayload(payload) {
  const direct = String(payload.repository || '').trim().toLowerCase();
  if (direct.includes('/')) return direct;

  const owner = String(payload.repository_owner || '').trim().toLowerCase();
  const name = String(payload.repo || payload.repository || '').trim().toLowerCase();
  if (owner && name) return `${owner}/${name}`;

  const sub = String(payload.sub || '');
  const match = sub.match(/^repo:([^:]+):/i);
  return match ? match[1].toLowerCase() : direct;
}
...
async function verifyGithubActionsToken(token) {
  ...
  if (!repository || repository !== expectedRepo) {
    throw new Error(`Unexpected GitHub repository: ${repository || payload.repository || 'unknown'}`);
  }
